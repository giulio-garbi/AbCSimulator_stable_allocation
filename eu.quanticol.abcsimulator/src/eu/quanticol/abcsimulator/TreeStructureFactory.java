...
public class TreeStructureFactory implements SimulationFactory<AbCSystem>{
	...
	public TreeStructureFactory(
		...
		this.mDemand = new int[leafs/2];
		this.wRating = new boolean[leafs/2];
		for(int i=0; i<leafs/2; i++) {
...
//		System.out.println(level);
		...
		for (ServerNode parent : level) {
			for( int i=0 ; i<agents && i < mDemand.length + wRating.length ; i++ ) {
				ComponentNode n;
				if(i < mDemand.length) {
					n = new ComponentNode(system, counter+i, parent,new SAInitiator(i, mDemand[i], wRating.length));
				} else  {
					n = new ComponentNode(system, counter+i, parent,new SAResponder(i - mDemand.length, wRating[i - mDemand.length]));
				}
				nodes.add(n);
